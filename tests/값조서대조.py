...
# 제3자 기특정 콜 — 발행자가 자산을 인식하지 않으므로 분개 차변이 100 이다 (주주간 분배)
CASES += [("equity", 2, "TF", 1, None, dict(_CHA, issuer_call=2, k_split=1, k_kind=1)),
          ("liability", 2, "TF", 1, None, dict(_CHA, issuer_call=2, k_split=1, k_kind=1)),
          ("equity", 2, "TF", 1, None, dict(_CHA, issuer_call=2, k_split=1, k_hold=0))]
