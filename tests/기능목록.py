...
MANIFEST = {
    ...
    "acc_basis":   ("행사금액 경과기간", {1: "계약 개월 ÷ 12 — 계약서가 세는 방식", 0: "Actual/365 — 할인기간과 같은 잣대(종전)"}, ("CB", "BW", "RCPS")),
    # 계약서의 회차별 표. 값이 아니라 «있음/없음» 이 갈래를 만든다.
    "p_cpn_add":   ("조기상환 행사일 이자 별도지급", {0: "행사금액만 — 산식·표가 이미 담고 있다", 1: "행사금액 + 그 날 이자"}, ("CB", "BW", "RCPS")),
    "k_cpn_add":   ("매도청구 행사일 이자 별도지급", {0: "행사금액만 — 산식·표가 이미 담고 있다", 1: "행사금액 + 그 날 이자"}, ("CB", "BW", "RCPS")),
    "p_sched":     ("조기상환 행사금액표", {"": "없음 — 산식으로 계산",
                                        "24 104.0000": "있음 — 표가 산식보다 우선"}, ("CB", "BW", "RCPS")),
    "k_sched":     ("매도청구 행사금액표", {"": "없음 — 산식으로 계산",
                                        "24 105.0000": "있음 — 표가 산식보다 우선"}, ("CB", "BW", "RCPS")),
    ...
    # 그 곡선이 직접 입력 칸에 들어오므로 같은 일을 두 번 묻던 갈래였다
    "rate_mode":   ("위험 곡선 출처", {"direct": "직접 입력 (고시표 적용 포함)",
