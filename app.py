...
SCOPE_NOTE = (
    "적용범위 — 주가를 관찰할 수 있는 **단일 종류**의 전환사채·상환전환우선주를 "
    "이항격자로 재고 전환권·상환청구권·매도청구권·리픽싱을 분리한다.")
UNMODELLED_NOTE = (
...
def remeasure(tm: Terms, rows):
    ...
    """
    fv_liab = sum(v for k, v in rows[:-1] if "파생상품부채" in k)
...
with st.sidebar:
    ...
    with st.expander("기본", expanded=True):
        t.S0 = st.number_input("평가기준일 주가 (원)", value=float(t.S0), step=1.0,
                               help="비상장이면 별도 지분평가액 ÷ 주식수를 넣거나, 아래에서 "
                                    "발행가로 역산하십시오.")
        ...
        if bc2.button("발행가로 주가 역산", use_container_width=True,
                      help="전체 가치(B2, 발행자 상환권이 있으면 B3)가 목표와 같아지는 "
                           "주가를 이분법으로 찾아 위 칸에 넣습니다 (책 [사례 5-1])."):
            with st.spinner("격자를 되풀이 계산합니다"):
                _S, _v, _k = backsolve(t)
            if _k < 0:
        ...
        if t.issuer_call == 1:
            ...
            t.k_lock = st.number_input(
                ...
                help="인수인이 콜옵션 대상주식을 **미전환 상태로 보유**해야 하는 기간입니다. "
                     ...
                     "**유무가치비교법에서만** 값에 들어갑니다.")
            # 옵션차익혼합할인법은 노드의 E·B 분해 위에 정의된 산식이라 TF 전용이다
            # (한공회 4.4.3). GS 를 고르면 기초상품만 GS 이고 콜은 TF 라 표시와
            # 계산이 어긋나므로 조합 자체를 막는다.
            _gs_blk = (t.model == "GS")
          ...
          # 위 RCPS 갈래와 같은 이유로 GS + 옵션차익혼합할인법은 막는다.
          _gs_blk = (t.model == "GS")
          if _gs_blk and t.k_method:
              t.k_method = 0
          t.k_method = st.selectbox("평가방법", [0, 1, 2],
...
with tabs[1]:
    ...
    je = ("[최초 인식]\n" + "\n".join(_ln)
          ...
          + ("차) 파생상품평가손익            매 결산 공정가치로 재측정\n"
             ...
             ("차) 파생상품평가손익            분리한 파생상품부채를 공정가치로 재측정\n"
              "    대) 파생상품부채\n" if any("파생상품부채" in k for k, _ in alloc_rows[:-1])
              else "")
    ...
    if not _rm["has"]:
        st.caption("결산 평가라면 사이드바 **기말 재평가 · 전기 장부금액** 에 전기말 장부금액을 "
                   ...
                   f"지금 파생상품부채 공정가치는 **{_rm['fv_liab']:,.4f}** 입니다.")
    else:
        _F = t.face_total/100
        _pl = _rm["pl"]
        st.dataframe(pd.DataFrame([
            ["파생상품부채 · 전기말 장부금액", _rm["prev"], _rm["prev"]*_F],
            ["파생상품부채 · 당기말 공정가치", _rm["fv_liab"], _rm["fv_liab"]*_F],
